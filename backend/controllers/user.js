...
exports.signup = (req, res, next) => {
  ...
  if (checkEmail(req.body.email) == null) {
    res.status(400).json({
      error: "L'email doit être du format suivant : toto@groupomania.fr",
    });
    /* Verification format mdp  */
  } else if (checkMdp(req.body.password) == null) {
    res.status(400).json({
      error:
        "Le mot de passe doit contenir au moins 8 caractères, dont au moins 1 chiffre, 1 lettre majuscule et un caractère spécial",
    });
  } else if (!checkName(req.body.firstname) || !checkName(req.body.lastname)) {
    res.status(400).json({
      error: "Le nom & prénom de doivent pas contenir de caractères spéciaux",
    });
  } else {
...
const checkEmail = (email) => {
  if (email.length > 0 && (email.length < 7 || email.trim() == "")) {
    ...
    return null;
  } else if (!email.match(/^[\w_\.-]+@groupomania.fr$/i)) {
...
const checkName = (value) => {
  if (!value.match(/^[a-zA-Z\-éÉèç ]*$/) || value.trim() == "") {
    return null;
  } else {
    return true;
  }
